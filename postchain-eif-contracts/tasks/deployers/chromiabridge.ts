...
// Used to upgrade the ChromiaTokenBridge contract to ChromiaTokenBridgeV2 to be able to set a new offset
task("upgrade:chromiabridge-to-v2")
  ...
  .setAction(async ({ address, offset, verify }, hre) => {
    ...
    console.log(`Upgrading ChromiaTokenBridge at ${address} to ChromiaTokenBridgeV2 with offset ${offset}`);
    ...
    const factory: ContractFactory = await hre.ethers.getContractFactory("ChromiaTokenBridgeV2");
    const bridgeV2: ChromiaTokenBridgeV2 = <ChromiaTokenBridgeV2>await hre.upgrades.upgradeProxy(
        bridgeV1.address, factory);

    console.log("ChromiaTokenBridge has been upgraded to ChromiaTokenBridgeV2");
    console.log(`Initializing ChromiaTokenBridgeV2 with offset ${offset}`);

    const tx = await bridgeV2.initializeV2(offset);
    await tx.wait();
    console.log("ChromiaTokenBridgeV2 initialized successfully");
