...
const nodeURLPool = process.env.REACT_APP_POSTCHAIN_URL || ""
const blockchainRID = process.env.REACT_APP_POSTCHAIN_BRID || ""
const rest = restClient.createRestClient([ nodeURLPool ], blockchainRID, 5);
...
const Bridge = ({ bridgeAddress, tokenAddress }: Props) => {
  ...
  const waitConfirmation = function (txRID: Buffer) {
        ...
        if (err) {
          ...
          switch (status) {
            ...
            case "waiting":
              ...
              break
            default:
              console.log('waitConfirmation default =>', {res, status});
  ...
  const postchainClaim = async () => {
    try {
      ...
      const pk = userPUB.toString('hex');
      // const pk = util.toBuffer(userPUB).toString('hex')
      const _signature = await signer.signMessage(pk);
      const signature = _signature.split('x')[1];
      var r = Buffer.from(signature.substring(0, 64), 'hex');
      var s = Buffer.from(signature.substring(64, 128), 'hex');
      var v = parseInt(signature.substring(128, 130), 16) - 27;
      ...
      if (tokenType === "ERC721") {
        ...
        const amount = BigNumber.from(withdrawAmount).mul(BigNumber.from(10).pow(unit)).toString();
  ...
  const depositTokens = async () => {
    ...
    try {
      ...
      const value = BigNumber.from(amount).mul(BigNumber.from(10).pow(unit));
      const calldata = bridge.interface.encodeFunctionData(
        "deposit",
        [tokenAddress, value, Buffer.from(accountId, 'hex')]
      )
