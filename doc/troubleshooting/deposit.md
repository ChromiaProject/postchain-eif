...
Check whether the Event Receiver chain has processed your transaction block, using the following CLI command:
...
4. For a deposit from a smart contract address, if the smart contract address is not linked to the FT4 account
   ```bash
...
7. If the FT4 account exists but is not in an access list
   ```bash
