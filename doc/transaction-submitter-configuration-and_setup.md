...
| `node_tx_verification_timeout`    | Time out for a transaction to be verified as processed on EVM side in milliseconds, if this deadline is passed the transaction is considered to have failed                                                                                               | int  |                    | 240000   |
...
| `evm.delayWhenNoNewBlocks` | Sleep timeout if no blocks received (milliseconds)                                                                                      | int  | 500     | `POSTCHAIN_TRANSACTION_SUBMITTER_EVM_DELAY_WHEN_NO_NEW_BLOCKS` |
