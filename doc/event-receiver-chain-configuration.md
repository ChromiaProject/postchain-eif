...
| `skip_to_height`  | The block number from which the Event Receiver will start querying events (usually equals block height at which the smart contract was deployed). If set to negative it will start at the given number from the latest block number, for example -1000 will start 1000 blocks behind the latest built block. | int           |                     | 0       |
...
| `${chain}.maxQueueSize`       | The size of the internal queue for blocks that have been fetched but not yet processed                                                                                                                                                                                                                                      | int          | 2000    | `POSTCHAIN_EIF_${CHAIN}_MAX_QUEUE_SIZE`        |
