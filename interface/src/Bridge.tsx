...
import ERC20TokenArtifacts from "./abis/erc20.json";
import ERC721TokenArtifacts from "./abis/erc721.json";
import BridgeArtifacts from "@eif/contracts/src/artifacts/contracts/TokenBridge.sol/TokenBridge.json";
...
const TokenInfo = ({ tokenAddress, bridgeAddress, tokenType, tokenId }: { tokenAddress: string, bridgeAddress: string, tokenType: string, tokenId: number}) => {
  ...
  const fetchTokenInfo = async () => {
    ...
    let withdraws;
    if (tokenType === "ERC721") {
      tokenContract = new ethers.Contract(tokenAddress, ERC721TokenArtifacts, library);
      ...
      tokenContract = new ethers.Contract(tokenAddress, ERC20TokenArtifacts, library);
...
const Bridge = ({ bridgeAddress, tokenAddress}: Props) => {
  ...
  const approveTokens = async () => {
    const signer = library.getSigner();
    try {
      const tokenContract = new ethers.Contract(tokenAddress, ERC20TokenArtifacts, library)
      const value = ethers.BigNumber.from(amount).mul(ethers.BigNumber.from(10).pow(unit))
  ...
  const setApprovalForAll = async () => {
    const signer = library.getSigner();
    try {
      const tokenContract = new ethers.Contract(tokenAddress, ERC721TokenArtifacts, library)
